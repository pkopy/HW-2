      ...
      if(tokenIsValid) {
        ...
        _data.create('carts', email, cart, (err) => {
          if(!err) {
            ...
            callback(500, {'Error': 'Could not create the cart for this user'})
          }
        });
      } else {
        callback(403, {'Error' : 'Missing required token in header, or token is invalid'});
...
//Carts - get
//Required data: email (cart name), token
//Optional data: none
        ...
        _data.read('carts', email, (err, cartData) => {
          if(!err && cartData) {
            if(cartData.items.length > 0) {
              const msg = helpers.createMessage(cartData.items)
              console.log(msg)
              helpers.sendEmailByMailGun(email, msg, (err) => {
                if(!err) {
                  callback(200)
                } else {
                  callback(403, {'Error' : 'Could not send email with order'})
                }
              });
            } else {
              callback(403, {'Error' : 'Your cart is empty'});
            }
          } else {  
            callback(404);
          }
...
//Add item to cart
handlers.add = (data, callback) => {
  const acceptableMethods = ['get'];
  if(acceptableMethods.indexOf(data.method) > -1) {
    handlers._add[data.method](data, callback);
  } else {
    callback(405, {'Error' : `You can use only GET method`});
  }
};


//Container for add submethods
handlers._add = {};

//Add - get
//Required data: token, item, qty, email
//Optional data: none
handlers._add.get = (data, callback) => {

  //Check that the item and qty is valid
  const email = typeof(data.payload.email) == 'string' && data.payload.email.trim().length > 0  && /[\w+0-9._%+-]+@[\w+0-9.-]+\.[\w+]{2,3}/.test(data.payload.email.trim()) ? data.payload.email.trim() : false;
  const item = typeof(data.queryStringObject.item) == 'string' && data.queryStringObject.item.trim().length > 0  ? data.queryStringObject.item.trim() : false;
  const qty = typeof(Number.parseInt(data.queryStringObject.qty)) == 'number' && data.queryStringObject.qty > 0  ? Number.parseInt(data.queryStringObject.qty) : false;
  ...
  if(item && qty && email) {
    ...
    handlers._tokens.verifyToken(token, email, (tokenIsValid) => {
      if(tokenIsValid) {
        _data.list('items', (err, itemsList) => {
          if(!err && itemsList && itemsList.length > 0 && itemsList.indexOf(item) > -1) {
            _data.read('carts', email, (err, cartData) => {
              if(!err && cartData) {
                
                //Get the price from item
                _data.read('items', item, (err, itemData) => {
                  if(!err && itemData) {
                    ...
                    if(cartData.items.length > 0) {
                      cartData.items.forEach((itemFromCart) => {
                        if(itemFromCart.item == item) {
                          itemFromCart.qty = qty; 
                          itemFromCart.value = qty * price
                          isItemExist = true;
                        } 
                    ...
                    _data.update('carts', email, cartData, (err) => {
                      if(!err) {
                        ...
                        callback(500, {'Error' : 'Cannot update the specified cart'});
                      }
                    });
                  } else {
                    callback(500, {'Error' : 'Cannot read the specified item'});
                  }
                });
              } else {
                callback(500, {'Error' : 'Cannot read the specified cart'});
              }
            }); 
          } else {
            callback(400, {'Error' : 'Send item does not exist'});
          }
        });
...
handlers.del = (data, callback) => {
  const acceptableMethods = ['delete'];
  if(acceptableMethods.indexOf(data.method) > -1) {
    handlers._del[data.method](data, callback);
  } else {
    callback(405, {'Error' : `You can use only DELETE method`});
  }
};
...
handlers._del.delete = (data, callback) => {
      ...
      if(tokenIsValid) {
        _data.read('carts', email, (err, cartData) => {
          if(!err && cartData) {
            ...
            if(cartData.items.length > 0) {
              let updatedCartItems = cartData.items.filter(itemFromCart => itemFromCart.item != item);
              ...
              callback(400, {'Error' : 'Cannot delete specified item'})
            }
          } else {
            callback(500, {'Error' : 'Cannot read the specified cart'});
          }
